#-------------------------------------------------------------------------------
# Title: ACTG 5248 analysis of CD8 T cell Memory Restoration
#
# Purpose: 
...
# People with HIV in the 1.5 years following ART initiation (ACTG5248)
#   - 10 participants
...
ACTG <- read.csv(file = paste0(curr_dir,'Data/updated_v4/ACTG CD8 subpopulations Event Counts_10 participants_v6_with FC.csv'))
#Modify here as data is updated
DS <- read.csv(file = paste0(curr_dir,'Data/updated_v4/CD8 CHI Durably Suppressed Event Counts_v5.csv'))
...
library(ggplot2)
pdf(file = paste0(curr_dir,"data/output/CD8_ACTG_PT_plots.pdf"))
...
#               broom::tidy(margins(glm(cbind(Event.count, 
...
#                                                     FC=first(FC.CD127.on.total.CD8.T.cells)) 
# 
...
# slopes_ACTG_df_sub<-slopes_ACTG_df[which(slopes_ACTG_df$Population=="CD127+"),]
# 
...
#    
# require(writexl)
# write_xlsx(merge.corr,paste0(curr_dir,"data/output/Spearman_Correlations_CD8.xlsx"))
...
fems_ACTG<-do(by_population_ACTG, 
               broom::tidy(margins(glm(cbind(Event.count, 
                  (Denominator - Event.count)) ~ -1 +  ThirtyDays + 
                  PID291374 + PID363043 + PID363044 + PID214273 + PID611175 + 
                  PID611183 + PID611051 + PID21929 + PID112185 + PID611172, 
                  family=quasibinomial, data = .),variables="ThirtyDays")))
...
fems_DS<-do(by_population_DS, 
              tidy(margins(glm(cbind(Event.count, 
                   (Denominator - Event.count)) ~ -1 + Months.post.timepoint.1+
                   + PID546 + PID861 + PID728 + PID674 + PID1095 + PID929
                   + PID749 + PID673 + PID870 + PID1036, 
                   family=quasibinomial, data = .),
...
require(writexl)
#Modify here as data is updated
write_xlsx(fems_ACTG, paste0(curr_dir,"data/output/CD8_ACTG_analysis_AMW_10participants_19May2023.xlsx"))
#Modify here as data is updated
write_xlsx(fems_DS, paste0(curr_dir,"data/output/CD8_DS_analysis_AMW_10participants_19May2023.xlsx"))
# 
# #----------------------------------------------------------------------
...
#              lmer(Percentages ~ Day + (1|PID), data = df))
# 
